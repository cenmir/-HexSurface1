function [surfh, surfX, SurfEle, nCutEle ] = CutP1(H, phi, level)
...
%         tol = 0.00001;
%         [~, ii] = sortrows(P);
%         ii_unique = ii(logical([1; any(diff(P(ii,:))>tol,2)]));
%         P = P(sort(ii_unique),:);
%         
%         if size(P,1) < 3
%             faultyElements = [faultyElements;iel];
%             continue;
%         end
